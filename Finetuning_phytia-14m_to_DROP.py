...
train_dataset = load_dataset("ucinlp/drop", split="train[:5]") # Initially only load 10% of the data, when everything works, please remove [:10%]
validation_dataset = load_dataset("ucinlp/drop", split="validation[:5]")
# Load model directly
...
def format_data(example):
    ...
    return {'input_text': input_text, 'target_text': input_text + answer}
...
print(formatted_validation_dataset[0])
...
def evaluate_model(model, tokenizer, dataset):
    ...
    for example in dataset:
        ...
        answer = tokenizer.decode(torch.tensor(example['labels']).to(device), skip_special_tokens=True).strip()

        # Debug prints
        if count<10:
          print(f"Input: {tokenizer.decode(input_ids[0], skip_special_tokens=True)}")
          print(f"Prediction: {prediction[len(tokenizer.decode(input_ids[0], skip_special_tokens=True)):]}")
          print(f"Answer: {answer[len(tokenizer.decode(input_ids[0], skip_special_tokens=True)):]}")
          print("-" * 50)
        count+=1
        if prediction[len(tokenizer.decode(input_ids[0], skip_special_tokens=True)):].lower() == answer[len(tokenizer.decode(input_ids[0], skip_special_tokens=True)):].lower():
            correct += 1
        total += 1
...
accuracy = evaluate_model(model, tokenizer, tokenized_train_dataset)
